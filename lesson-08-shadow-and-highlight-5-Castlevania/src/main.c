...
void myJoyHandler( u16 joy, u16 changed, u16 state)
{
	if (joy == JOY_1)
	{
        ...
        {
            estado = 2;
            SPR_setPriority(spr_Morris, TRUE);
        }
        ...
        {
            estado = 3;
            SPR_setPriority(spr_Muerte, TRUE);
        }
        ...
        // en la posici�n que debe ocupar en pantalla (no pintamos la pantalla entera)
        ...
        if(estado == 5 && (changed & BUTTON_C) )
        {
            ...
            SPR_setPriority(spr_Morris, FALSE);
            //Cambio de animacion
        ...
        if(estado == 9 && (changed & BUTTON_A) )
        {
            ...
            TileMap *shadowmap_tilemap = fondo_doble_prioridad.tilemap;
        ...
        if(estado == 10 && (changed & BUTTON_B) )
        {
            ...
            // para cada l�nea, empezando desde la cero (0) con un offset determinado (aux).
            // El numero de lineas es (NUM_LINES) en este caso toda la pantalla, y (1) es
            // c�mo hacerlo, en este caso usando la CPU, y por tanto esto se har� durante el
            // per�odo donde no se dibuja en pantalla
            VDP_setHorizontalScrollLine(BG_A, 0, aux, NUM_LINES, 1);
                //al desplazar TODO el plano A, el texto tambi�n sale "en diagonal"
