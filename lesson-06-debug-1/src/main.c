...
int main( bool hard ) {
    ...
    while ( TRUE ) {
        ...
        //Utilizamos primero cadenas de texto "normales", despu�s
        //usamos strings, luego convertimos n�meros a cadenas y
        ...
        //imprime un n�mero entero positivo,antes debemos convertirlo a texto
        int mi_numero = 1200;
        intToStr(mi_numero,char_salida,1);
        KDebug_Alert(char_salida);

        //imprime un n�mero entero negativo
        mi_numero = -1200;
        intToStr(mi_numero,char_salida,1);
        KDebug_Alert(char_salida);

        //imprime un n�mero en punto fijo positivo
        fix16 mi_numero16 = FIX16(5.4);
        fix16ToStr(mi_numero16,char_salida,1);
        KDebug_Alert(char_salida);

        //imprime un n�mero en punto fijo negativo
        ...
        while ( TRUE ) { value = JOY_readJoypad(JOY_1); if (value & BUTTON_B) break; }
        ...
        while ( TRUE ) { value = JOY_readJoypad(JOY_1); if (value & BUTTON_C) break; }
        ...
        VDP_drawText("Pulsa -A- para continuar               ", 1, 20);
        while ( TRUE ) { value = JOY_readJoypad(JOY_1); if (value & BUTTON_A) break; }
